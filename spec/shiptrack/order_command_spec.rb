...
module ShipTrack
  ...
  describe OrderCommand do
    ...
    describe '.initialize' do
      ...
      it 'has a name' do
        expect( command.name ).to eq 'order'
      end
    ...
    describe '#run' do
      ...
      it 'sets the order date on the shipment' do
        shipment.should_receive( :order_date= ).with( DateTime.now.strftime( '%Y-%m-%d' ) )
        command.run( params, configuration, {} )
      end
